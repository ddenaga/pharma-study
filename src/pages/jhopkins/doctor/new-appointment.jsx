...
export default function ScheduleAppointment(props) {
	...
	const handleSubmit = (values, { setSubmitting }) => {
		...
		setAppointments([...appointments, { ...values, id: new Date().getTime() }]);
					...
					{({ isSubmitting, setFieldValue }) => (
						<Form className="space-y-4">
							<div className="flex flex-col">
								...
								<Field as="select" name="patientId" className="border p-2">
												...
												<div className="grid grid-cols-4 gap-2">
													{timeSlots.map((time, index) => (
														<div
															...
															className={`rounded border p-2 text-center ${selectedTime === time ? 'bg-blue-500 text-white' : ''
																}`}
														>
															{time}
														</div>
